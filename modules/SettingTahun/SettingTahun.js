...
(function () {
    ...
    MyApp.$me.selectorTable = $('#datatable_fixed_column').DataTable({
        ...
        "columns": [
            ...
            {
                "data": function (val, item, index) {
                    return val.aktif == '1' ? '<span class="label label-success">Aktif</span>' : '<span class="label label-danger">Tidak Aktif</span>';
                }
            }
        ],
        dom: 'Bfrtip',
        ...
        initComplete: function () {
            ...
            selectortable.on('click','.btHapus',function () {
                index = $(this).data('index');
                dataRow = MyApp.$me.dataRow[index];
                konfirmasi = confirm('Apakah anda yakin ingin menghapus tahun '+dataRow.tahun);
                if(konfirmasi){
                    hapus(dataRow);
                }
            });
            ...
            if (resp.success) {
                MyApp.loadModuleFile('template/tpl_pdf.html', function (tpl) {
                    ...
                    for (i=0;i<resp.result.length;i++){
                        data2[i]={};data2[i].aktif = nama=resp.result[i].aktif;data2[i].id_tahun = nama=resp.result[i].id_tahun;data2[i].tahun = nama=resp.result[i].tahun;
                    }

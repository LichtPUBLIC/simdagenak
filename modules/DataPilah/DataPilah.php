...
class DataPilah extends Database
{
    ...
    public function ACTION_addKecamatanKemendagri(){
        $params = isset($_GET) ? $_GET : $_POST;
        if(isset($_GET['kode_data_pilah'])) {
            $params = $_GET;
        }
        ...
        $count = $this->dbDataGetValue($sqlCek);
        if ($count > 0) {
            echo json_encode(array("success" => false, "msg" => "Daftar baris harus kosong untuk menggunakan 17 kecamatan Kemendagri."));
            return;
        }

        $kecamatanSleman = array(
            "Gamping", "Godean", "Moyudan", "Minggir", "Seyegan", "Mlati", "Depok", "Berbah",
            "Prambanan", "Kalasan", "Ngemplak", "Ngaglik", "Sleman", "Tempel", "Turi", "Pakem", "Cangkringan"
        );
        ...
        echo json_encode(array("success" => true, "msg" => "Berhasil menambahkan 17 kecamatan Sleman Kemendagri."));
    }
